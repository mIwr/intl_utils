...
void main() {
  group('Path validation', () {
    test(
      'Test path validation with blank string',
      () => expect(isValidPath('  '), isFalse),
    );

    test(
      'Test path validation with forbidden path character *',
    ...
    test(
      'Test path validation with Windows path',
      () => expect(isValidPath(r'lib\l10n'), isTrue),
    );

    test(
      'Test path validation with UNIX path',
      () => expect(isValidPath('lib/l10n'), isTrue),
    );

    test(
      'Test path validation with escaped path separators',
      () => expect(isValidPath('lib\\l10n'), isTrue),
    );

    test(
      'Test path validation with dual path separators',
      () => expect(isValidPath('lib//l10n'), isTrue),
    );

    test(
      'Test path validation with Windows absolute path',
  ...
  group('Download empty as param validation', () {
    test(
      'Test download empty as param with empty string',
      () => expect(isValidDownloadEmptyAsParam(''), isFalse),
    );

    test(
      'Test download empty as param with blank string',
      () => expect(isValidDownloadEmptyAsParam('  '), isFalse),
    );

    test(
      'Test download empty as param with unsupported value',
      () => expect(isValidDownloadEmptyAsParam('unsupported'), isFalse),
    );

    test(
      'Test download empty as param with invalid empty value',
      () => expect(isValidDownloadEmptyAsParam('emty'), isFalse),
    );

    test(
      'Test download empty as param with empty value',
      () => expect(isValidDownloadEmptyAsParam('empty'), isTrue),
    );

    test(
      'Test download empty as param with invalid main value',
      () => expect(isValidDownloadEmptyAsParam('mmain'), isFalse),
    );

    test(
      'Test download empty as param with main value',
      () => expect(isValidDownloadEmptyAsParam('main'), isTrue),
    );

    test(
      'Test download empty as param with invalid skip value',
      () => expect(isValidDownloadEmptyAsParam('skipp'), isFalse),
    );

    test(
      'Test download empty as param with skip value',
  ...
  group('Locale validation', () {
    test(
      'Test locale validation with empty string',
      () => expect(isValidLocale(''), isFalse),
    );

    test(
      'Test locale validation with blank string',
      () => expect(isValidLocale('  '), isFalse),
    );

    test(
      'Test locale validation with plain text',
